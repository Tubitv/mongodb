defmodule Mongo.Auth.SCRAM do
  ...
  defp generate_proof(salted_password, auth_message) do
    client_key = hmac_sha(salted_password, "Client Key")
    stored_key = :crypto.hash(:sha, client_key)
    signature = hmac_sha(stored_key, auth_message)
  ...
  defp generate_signature(salted_password, auth_message) do
    server_key = hmac_sha(salted_password, "Server Key")
    hmac_sha(server_key, auth_message)
  end
  ...
  if Code.ensure_loaded?(:crypto) and function_exported?(:crypto, :mac, 4) do
    defp hmac_sha(secret, data), do: :crypto.mac(:hmac, :sha, secret, data)
  else
    defp hmac_sha(secret, data), do: :crypto.hmac(:sha, secret, data)
  end
end
